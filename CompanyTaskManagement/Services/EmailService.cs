...
namespace CompanyTaskManagement.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<bool> SendOverdueTasksNotificationToHrAsync(IEnumerable<TaskItem> overdueTasks)
        {
            try
            {
                ...
                var hrEmail = !string.IsNullOrWhiteSpace(_emailSettings?.HrEmailAddress)
                ...
                sb.AppendLine("<p>Target Recipient: HR Department (" + hrEmail + ")</p>");
                sb.AppendLine("</div></div></body></html>");

                return await SendEmailAsync(hrEmail, subject, sb.ToString());
            }
            catch (Exception ex)
            {
